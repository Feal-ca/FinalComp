...
int main(int argc, char **argv){
	...
	len = sprintf(buf, "Checkpoint 1:\nThe variable pointer ptr is located at %p and now it holds the address %p\nPress Return to continue...", &ptr, ptr); //&ptr: l'adreça del pointer(on es guarda el pointer)
	write(1, buf, len);							//*ptr: valor del què apunta el pointer(l'int)
	read(0, buf, 1);							//ptr és l'adreça que guarda el pointer (l'adreça de l'int)

	ptr = &globalVar;
	*ptr = 123;
	//MISSING CODE: substitute CCC, DDD, EEE, FFF
	len = sprintf(buf, "Checkpoint 2:\nThe ptr now points to %p, that is the same address than globalVar %p\nThus, both have the same integer value %d vs %d\nPress Return to continue...", ptr, &globalVar, *ptr, globalVar);
	write(1, buf, len);
	read(0, buf, 1);
